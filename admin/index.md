---
layout: home
title: WISE Administrator Guide
---

- How to set up WISE on a production server
  - [Using Docker](install-production-server-docker/) (recommended)
  - [Using Tomcat](install-production-server-tomcat/)
- [Sample server architecture](server-architecture)
- [Transfer units between instances](transfer-units/)
- [Enable AI Chat feature](enable-ai-chat-feature/)
- [Enable Speech-to-Text transcription in units](enable-speech-to-text-transcription/)
- [Application updates](updates/)
- [Unit Recovery Mode](unit-recovery-mode/)
